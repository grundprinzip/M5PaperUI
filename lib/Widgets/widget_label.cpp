...
void Label::Init() {
  ...
  canvas->setTextDatum(convert_alignment());
  canvas->setTextSize(text_size_);
  canvas->setTextColor(Grayscale::G15);
  ...
  canvas->drawString(text_.c_str(), border_offset, border_offset);
}

void Label::Draw() { Widget::Draw(); }
